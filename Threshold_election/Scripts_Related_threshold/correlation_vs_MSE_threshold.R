...
for (i in 1:length(substrates)) {
  ...
  statisticslm_df <- data.frame(cor_value,lm_counts, lm_MSE, lm_correlation)
  statisticslme_df <- data.frame(lme_counts, lme_MSE, lme_correlation)
  
  # Combine data frames and calculate difference
  join_df <- cbind(statisticslm_df, statisticslme_df)
  join_df$diference <- join_df$lme_correlation - join_df$lm_correlation
  ...
  ggsave(filename = paste("MSE_Comparison_", substrates[i],"_5000_5",".png", sep = ""), plot = MSE_PLOT, width = 8, height = 6)
  ggsave(filename = paste("Correlation_Comparison_", substrates[i],"_5000_5", ".png", sep = ""), plot = CORRELATION_PLOT, width = 8, height = 6)
  
  
  # Save dataframe as CSV file
  write.csv(join_df, file = paste("Statistics_", substrates[i],"_5000_5",".csv", sep = ""), row.names = FALSE)
