...
matched_esv_file <- "matched_ESV_id_0.97.tsv"
matched_esv_path <- normalizePath(path.join("../genome_aligment/",matched_esv_file))

setwd(this.dir())
matched_asv_df <- read_tsv(matched_esv_path,col_names = F)
...
png(file = "median_overlap_family.png", width = 700, height = 500)
ggplot(grouped_family_df,aes(F1,median_overlap,colour =  coherence)) +
  geom_point(size = 7) +
  geom_hline(aes(yintercept = base_overlap_same_f, linetype = "Base overlap"), color = "black") +
  scale_linetype_manual(name = "", values = "dashed") +
  theme_classic() +
  ylab("Median overlap") +
  xlab("Family") +
  theme(axis.text.x = element_text(face = "bold",angle = 90,hjust = 1,vjust = 0.5,size = 10), 
  axis.text.y = element_text(face = "bold",size = 12) ,  axis.title.y = element_text(face = "bold",size = 17),
  legend.title = element_text(face = "bold", size = 17) , legend.text = element_text(size = 17),
  axis.title.x = element_text(face = "bold", size = 17)) +
  scale_color_discrete(name = "Coherency", labels = c("Early coherent" , "Incoherent", "Late coherent"))
  # theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust = 0.5,size = 10))
dev.off()

png(file = "overlap_family.png", width = 700, height = 500)
...
# --- 3. Boxplot: normalized_difference ---
png(file = "normalized_difference_family.png", width = 700, height = 500)
ggplot(same_family_df, aes(F1, normalized_difference, colour = coherency)) +
  geom_boxplot() +
  geom_hline(aes(yintercept = base_difference_same_f, linetype = "Base difference"), color = "black") +
  scale_linetype_manual(name = "", values = "dashed") +
  theme_classic() +
  ylab("Normalized difference") +
  xlab("Family") +
  theme(
    axis.text.x = element_text(face = "bold", angle = 90, hjust = 1, vjust = 0.5, size = 10),
  ...
  scale_color_discrete(name = "Coherency",
                       labels = c("Early coherent", "Incoherent", "Late coherent"))
dev.off()


# --- 4. Scatter: median_difference ---
png(file = "median_difference_family.png", width = 700, height = 500)
...
png(file = "density_overlap_family.png", width = 700, height = 500)

ggplot() +
  # Curvas de densidad con transparencia
  geom_density(
    data = combined_data, 
    aes(x = overlap, fill = group),
    alpha = 0.5
  ...
  geom_vline(
    data = data.frame(mean = vline_means, group = names(vline_means)),
    aes(xintercept = mean, color = group),
    linetype = "dotted", size = 1.2
  ) +
  # Escalas de color coherentes
  scale_fill_manual(
    name = "Group",
    values = c("Different Family" = "yellow", "Same Family" = "brown"),
    labels = c("Different Family", "Same Family")
  ) +
  scale_color_manual(
    name = "Mean value",
    values = vline_colors,
  ...
  # Etiquetas y tema visual uniforme
  theme_classic() +
  xlab("Overlap") +
  ylab("Density") +
  theme(
    axis.text.x = element_text(face = "bold", angle = 0, hjust = 0.5, vjust = 0.5, size = 10),
    axis.text.y = element_text(face = "bold", size = 12),
    axis.title.x = element_text(face = "bold", size = 17),
    axis.title.y = element_text(face = "bold", size = 17),
    legend.title = element_text(face = "bold", size = 17),
...
# --- 1. Median overlap para different families ---
png(file = "median_overlap_diff_family.png", width = 700, height = 500)
ggplot(different_f_df_grouped, aes(Coherency, median_overlap, colour = Coherency)) +
  geom_point(size = 7) +  # tamaño fijo
  geom_hline(aes(yintercept = base_overlap_diff_f, linetype = "Base overlap"), color = "black") +
  scale_linetype_manual(name = "", values = "dashed") +
  scale_color_discrete(name = "Coherency",
                       labels = c("Early coherent - Early coherent", "Early coherent - Incoherent", "Early coherent - Late coherent",
                      "Incoherent - Incoherent", "Incoherent - Late coherent", "Late coherent - Late coherent")) +
  scale_x_discrete( labels = c("Early coherent - Early coherent", "Early coherent - Incoherent", "Early coherent - Late coherent",
                      "Incoherent - Incoherent", "Incoherent - Late coherent", "Late coherent - Late coherent")) +
  theme_classic() +
  ylab("Median overlap") +
  xlab("Coherency") +
  theme(
    axis.text.x = element_text(face = "bold", angle = 90, hjust = 0.5, vjust = 0.5, size = 10),
    axis.text.y = element_text(face = "bold", size = 12),
    axis.title.x = element_text(face = "bold", size = 17),
    axis.title.y = element_text(face = "bold", size = 17),
    legend.title = element_text(face = "bold", size = 17),
...
# --- 2. Median normalized difference para different families ---
png(file = "median_normalized_difference_diff_family.png", width = 700, height = 500)
ggplot(different_f_df_grouped, aes(Coherency, median_normalized_difference, colour = Coherency)) +
  geom_point(size = 7) +  # tamaño fijo
  geom_hline(aes(yintercept = base_difference_diff_f, linetype = "Base difference"), color = "black") +
  scale_linetype_manual(name = "", values = "dashed") +
  scale_color_discrete(name = "Coherency",
                       labels = c("Early coherent - Early coherent", "Early coherent - Incoherent", "Early coherent - Late coherent",
                      "Incoherent - Incoherent", "Incoherent - Late coherent", "Late coherent - Late coherent")) +
  theme_classic() +
  ylab("Median normalized difference") +
  xlab("Coherency") +
  scale_x_discrete( labels = c("Early coherent - Early coherent", "Early coherent - Incoherent", "Early coherent - Late coherent",
                      "Incoherent - Incoherent", "Incoherent - Late coherent", "Late coherent - Late coherent")) +
  theme(
