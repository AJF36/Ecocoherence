...
strategy_family_table <- read_tsv(file = "ecological_strategies_families.tsv" ,col_names = T)

### Load the table with the information of the ecological strategy of the modules
strategy_module_table <- read_tsv(file = "ecological_strategies_modules.tsv" ,col_names = T)
...
coherence_family_table_path <- normalizePath(file.path("../Z_score/z_score_families.tsv"))
...
coherence_module_table_path <- normalizePath(file.path("../Z_score/z_score_modules.tsv"))
...
for (sub in substrates){
  ...
  table_path  <- file.path("../modules_table",paste("group_analysis_",sub,"_TEST.tsv", sep = ""))
