...
library(this.path)
library(robCompositions) ##Package with the function for the Aitchinson function
library(zCompositions)##Package to replace the NA
...
margins = c(12, 8)
setwd(figures_dir)
pdf("heatmap_modules_families_all_modules_no_incoherent3.pdf", width = 10, height = 8)
#
...
dev.off()
pdf("heatmap_modules_families_all_modules_no_scale_2.pdf", width = 10, height = 8)
#
# # png("heatmap_high_res_modules.png", width = 4000, height = 2000, res = 300)
heatmap.2(
...
#### 
# Convertir a matriz numérica y trasponer si quieres distancias entre columnas
test_matrix <- as.matrix(all_modules_df[,-1])
test_matrix
# Reemplazar ceros (evita usar NA)
test_matrix_nozeros <- zCompositions::cmultRepl(t(test_matrix), label=0)  # usa t() si lo necesitas en forma original

# Calcular la distancia de Aitchison
dist_matrix <- robCompositions::aDist(test_matrix_nozeros)
dist_matrix
dist_matrix_for_view <- as.matrix(dist_matrix)
...
pdf("large_dendogram_atchinson_2.pdf", width = 40, height = 35, pointsize = 9)
par(mar = c(33, 0, 5, 5) ,oma = c(33, 0, 2, 2))  # Increased bottom margin for color bar
# Plot the dendrogram
dendogram <- dendogram  %>% dendextend::set("labels_cex" , 1) %>% 
  dendextend::set("branches_k_color",k=14) %>%
  dendextend::set("labels_col",k=14) %>%
  dendextend::set("branches_lwd",5) %>%
  dendextend::set("labels_cex",3.5) 
plot(dendogram)
dendogram %>% rect.dendrogram(k=14, border = 1,lty = 4, lwd = 4 ,lower_rect = 0)
# plot(dendogram, 
...
early_modules <- cluster_labels[[3]]

df_early_modules <- dplyr::select(all_modules_df, c("Family",early_modules))

colSums(df_early_modules[,-1])

late_modules <- cluster_labels[[2]]

df_late_modules <- dplyr::select(all_modules_df, c("Family",late_modules))
...
early_abundace_plot <- plot_bar(phylo_obj_early, fill = "Family") +
  ...
  theme_classic() +
  xlab("Early modules") +  # Example axis label
...
late_abundace_plot <- plot_bar(phylo_obj_late, fill = "Family") +
  ...
  theme_classic() + # Apply the common color palette
  xlab("Late Modules") +  # Example axis label
  ylab("Relative Abundance")  +
  theme(
    axis.text.x = element_text(face = "bold", size = 7, angle = 90),
    axis.text.y = element_text(face = "bold", size = 7),
...
setwd(relative_abundance_dir)

abundance_modules_df <- read_tsv("all_modules_relativeabundance.tsv") ###  Tabla con las abundancias

# anyDuplicated(colnames(abundance_modules_df)) ### Algun duplicado en las columnas? No

# test <- abundance_modules_df %>% #### Hay filas duplicadas , por eso el bug
#   count(module)



# abundance_modules_df <- abundance_modules_df %>%   ### Por algun motivo no reconoce module como columna
#   rename_with(make.unique) %>%
#   group_by(module) %>%
#   summarise(across(-module, mean), .groups = "drop")
...
abundace_early_modules_df <- abundace_early_modules_df %>%
  group_by(module, Time) %>%
  summarise(rel_abundance = mean(rel_abundance, na.rm = TRUE), .groups = "drop")



early_plot <- ggplot(abundace_early_modules_df, 
                     aes(x = as.numeric(Time), y = rel_abundance, colour = module)) +
  geom_line(size = 2) +
  theme_classic() +
  ylab("Relative abundance") +
  xlab("Time (h)") +
  labs(colour = "Early modules") +
  theme(
    axis.text.x = element_text(face = "bold", size = 7, angle = 90),
    axis.text.y = element_text(face = "bold", size = 7),
    legend.title = element_text(face = "bold", size = 12),
...
abundace_late_modules_df <- abundace_late_modules_df %>%
  group_by(module, Time) %>%
  summarise(rel_abundance = mean(rel_abundance, na.rm = TRUE), .groups = "drop")
...
#              colour = "red", size = 3)
late_plot <- ggplot(abundace_late_modules_df, 
                    aes(x = as.numeric(Time), y = rel_abundance, colour = module)) + 
  geom_line(size = 2) + 
  scale_colour_manual(values = discrete_palette) +
  theme_classic() +
...
pdf("Relative_abundance_composition_modules_all_modules_3.pdf", 12,8)
