...
script_dir <- this.dir()
results_dir <- normalizePath(file.path(script_dir, "..", "results", "8a_compute_zscore_families"), mustWork = FALSE)
figures_dir <- normalizePath(file.path(script_dir, "..", "figures", "8a_compute_zscore_families"), mustWork = FALSE)
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(figures_dir, recursive = TRUE, showWarnings = FALSE)
randomized_dir <- normalizePath(file.path(script_dir, "..", "results", "6e_compute_randomized_null_entropy_families"), mustWork = FALSE)
observed_dir <- normalizePath(file.path(script_dir, "..", "results", "7a_compute_observed_entropy_families"), mustWork = FALSE)
...
for (x in substrates) {
  ...
  observed_data <- read_tsv(file.path(observed_dir, paste(x,"observed_S_X_family.tsv",sep = "_")))
