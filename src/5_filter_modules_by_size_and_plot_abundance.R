...
script_dir <- this.dir()
results_dir <- normalizePath(file.path(script_dir, "..", "results", "5_filter_modules_by_size_and_plot_abundance"), mustWork = FALSE)
substrate_results_dir <- normalizePath(file.path(results_dir, substrate), mustWork = FALSE)
dir.create(substrate_results_dir, recursive = TRUE, showWarnings = FALSE)
...
# dirOut=paste(dir.funcInk,"figures",sep="/")
dirOut <- file.path(script_dir,"..","figures","5_filter_modules_by_size_and_plot_abundance",substrate)
...
subs_path <- file.path(script_dir,"..","results","4a_detect_modules_functionink",substrate)
...
sum(part.count > 4)
part.count
fileOut=file.path(substrate_results_dir,paste(fileFun,"_guildGT",min.module,".txt",sep=""))
fileOut
...
fileOut=file.path(substrate_results_dir,paste("otu_table_",substrate,"_byModulesSize",min.module,".tsv",sep=""))
