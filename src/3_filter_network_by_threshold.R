...
script_dir <- this.dir()
results_dir <- normalizePath(file.path(script_dir, "..", "results", "3_filter_network_by_threshold"), mustWork = FALSE)
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)
setwd(results_dir)
###Load the thresholds file
thres_file <- file.path(script_dir,"..","results","2_select_correlation_threshold","minimun_correlations_threshold.tsv")
...
for (substrate in substrates){
  net_file <- file.path(script_dir,"..","results","1_filter_asv_table_and_build_sparcc_network",substrate,paste("interactions_filtered_0.01._",substrate,"_.tsv", sep = ""))
