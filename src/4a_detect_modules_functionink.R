...
script_dir <- this.dir()
results_dir <- normalizePath(file.path(script_dir, "..", "results", "4a_detect_modules_functionink"), mustWork = FALSE)
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)
path_to_input <- file.path(script_dir,"..","results","3_filter_network_by_threshold")
path_to_input <- normalizePath(path_to_input)
setwd(results_dir)
for (substrate in substrates){
  substrate_dir <- file.path(results_dir,substrate)
  ...
  destiny <- file.path(results_dir,substrate)
  origin <- file.path(script_dir,"..","results","3_filter_network_by_threshold","functionink_tmp")
