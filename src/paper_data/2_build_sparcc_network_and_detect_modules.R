...
script_dir <- this.dir()
observed_data_dir <- normalizePath(file.path(script_dir, "..", "..", "results", "paper_data", "1_filter_asv_table"), mustWork = FALSE)
results_dir <- normalizePath(file.path(script_dir, "..", "..", "results", "paper_data", "2_build_sparcc_network_and_detect_modules"), mustWork = FALSE)
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)
file.copy(file.path(observed_data_dir, "data_filtered_for_sparcc_paper_data.tsv"), results_dir, overwrite = FALSE)
...
setwd(file.path(results_dir, "bootstrap_correlation"))
system("rm cov*")
...
## Functionink
  ...
  pathNet <- results_dir
  pathRepo <- file.path(Sys.getenv("HOME"), "functionInk")
