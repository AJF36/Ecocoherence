...
library(purrr)
script_dir <- this.dir()
results_dir <- normalizePath(file.path(script_dir, "..", "results", "6f_compute_randomized_null_entropy_modules"), mustWork = FALSE)
...
for (iteration in 1:50) {
  ...
  for (substrate in substrates) {
    ...
    ### This code eliminates the families that dont have at least 4 members, so the analysis is parallel with the no randomized
    ### If we take out the restriction delete this code
    functionink$family <- taxonomy$Family[match(functionink$ESV,row.names(taxonomy))]

    number_of_members_of_families <- map(functionink$family, \(x) sum(functionink$family == x))
    names(number_of_members_of_families) <- functionink$family
    index_number_of_big_families <- names(number_of_members_of_families[number_of_members_of_families >= 4])
    
    functionink <- filter(functionink, family %in% unique(index_number_of_big_families))
    ##
    ...
    ### For the module analysis we need the transpose matrix
    guild_matrix <- t(guild_matrix)
...
for (i in 1:length(results_list)) {
  ...
  for (j in 1:length(substrate_df_list)) {
    ...
    substrate_df <- substrate_df %>%
      mutate(across(
        .cols = !matches("members|Family"), ## change this so it only matches columns that dont contain numbers
    ...
    numeric_columns <- substrate_df %>% select(!matches("Family|members"))
...
write_tsv(final_summary,"randomized_entropy_by_modules.tsv")
